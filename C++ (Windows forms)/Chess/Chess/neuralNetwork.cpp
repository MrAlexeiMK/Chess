...
void neuralNetwork::log(double x) {
    LOG += to_string(x);
}
void neuralNetwork::log(vector<double> x) {
...
void neuralNetwork::log(matrix A) {
    for (int i = 0; i < A.N; ++i) {
        for (int j = 0; j < A.M; ++j) {
            log(A(i, j).real);
            LOG += " | ";
...
void neuralNetwork::save_log(string path) {
    ofstream out;
    out.open(path);
    out << LOG;
}
...
matrix neuralNetwork::activation_function(matrix x) {
    ...
    for(int i = 0; i < x.N; ++i) {
        for(int j = 0; j < x.M; ++j) {
            temp(i, j) = activation_function(x(i, j).real);
        }
    }
    return temp;
}
...
vector<double> neuralNetwork::query(vector<double> inputs_list) {
    ...
    for(int i = 0; i < outputs.N; ++i) {
        for(int j = 0; j < outputs.M; ++j) {
            res.push_back(outputs(i, j).real);
        }
    }
    return res;
}
