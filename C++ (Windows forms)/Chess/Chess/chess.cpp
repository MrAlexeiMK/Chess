...
chess::chess(vector<vector<char>> start_pos) {
    srand(time(NULL));
    pos = start_pos;
    coords king1 = {7, 4}, king2 = {0, 4};
    vector<step> st;
...
chess::chess(vector<vector<char>> start_pos, properties props, bool virt = false) {
    pos = start_pos;
    isVirtual = virt;
    prop = props;
    MAX_STEPS = 1000;
...
vector<vector<char>> chess::getDefaultPos() {
...
vector<vector<char>> chess::getReversePos() {
    vector<vector<char>> res = pos;
    for(int i = 0; i < 8; ++i) {
        for(int j = 0; j < 8; ++j) {
            res[i][j] = pos[7-i][7-j];
        }
...
double chess::getCoefficient() {
    double c1 = 0.0, c2 = 0.0;
    for (int i = 0; i < 8; ++i) {
        for (int j = 0; j < 8; ++j) {
            coords v = { i, j };
            if (isWhite(v)) c1 += fig_values[pos[i][j]];
            else if(isBlack(v)) c2 += fig_values[pos[i][j]];
        }
    }
    return (double)((double)c1 / (c1 + c2));
}
...
string chess::getStringPos(bool reverse) {
    string res = "";
    if (!reverse) {
        for (int i = 0; i < 8; ++i) {
            for (int j = 0; j < 8; ++j) {
        ...
        vector<vector<char>> rev = getReversePos();
        for (int i = 0; i < 8; ++i) {
            for (int j = 0; j < 8; ++j) {
                res += rev[i][j];
            }
...
string chess::getStatus() {
    if (prop.repeat >= 6 || resources.size() > MAX_STEPS || (current_figs.size() == 1 && opposite_figs.size() == 1)) {
        current_steps.clear();
        return "draw";
    }
...
void chess::trainAI(neuralNetwork& AI) {
    ...
    for (int i = 0; i < training_data.size(); ++i) {
        AI.train(training_data[i], vector<double>{(double)(resources[i] + K) / 2.0});
    }
}
