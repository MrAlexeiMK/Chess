...
namespace Chess {
	...
	private: System::Media::SoundPlayer^ step_sound;
	public:
	public:
		System::Windows::Forms::ProgressBar^ progress;
	private: System::ComponentModel::BackgroundWorker^ worker_ai;
	...
	public:
		...
		bool training = false;
	public:
		play(Form^ fr1, int t, bool white, string p, System::Windows::Forms::ProgressBar^ pb, bool tig, int rat, System::Windows::Forms::Label^ rating_label) {
			youWhite = white;
			otherform = fr1;
			type = t;
			*path = p;
			...
			if (type == 1 || type == 2) {
				try {
					AI->loadW("weights/" + *path);
				}
				catch (...) {
					AI->saveW("weights/" + *path);
				};
			}
			if (type == 1 && !white) worker_ai->RunWorkerAsync();
			dropColor();
			update();
			if (type == 2) simulate();
		}
	...
	private: System::Drawing::Image^ default_image;
	...
	private: System::ComponentModel::Container^ components;
		...
		void update() {
			string pos = "";
			if (youWhite) pos = game->getStringPos(false);
			else pos = game->getStringPos(true);
			//this->log->Text = gcnew System::String(pos);
			for (int i = 0; i < this->board->Controls->Count; ++i) {
				this->board->Controls[i]->BackgroundImage = nullptr;
				string num = to_string(i);
				System::String^ name = gcnew System::String(num.c_str());
				this->board->Controls[i]->Name = name;
				this->board->Controls[i]->Cursor = Cursors::Hand;
				if (pos[i] != ' ') {
					string file = "icons/" + string(1, pos[i]) + ".png";
					System::String^ s = gcnew System::String(file.c_str());
					this->board->Controls[i]->BackgroundImage = Image::FromFile(s);
				}
			}
		}
		void dropColor() {
		...
		int new_rating(int rat, int steps, double f, bool uWhite, string status) {
			if (steps <= 1000) {
				if (uWhite && status == "white") {
					return (int)(rat + (1 + (int)(1000 - steps) / 200) + (int)5 * f);
				}
				else if (!uWhite && status == "black") {
					return (int)(rat + (1 + (int)(1000 - steps) / 200) + (int)5*(1.0-f));
				}
				else if (uWhite && status == "black") {
					return (int)(rat - ((1 + (int)(1000 - steps) / 200) + (int)5 * f));
				}
				else if (!uWhite && status == "white") {
					return (int)(rat - ((1 + (int)(1000 - steps) / 200) + (int)5 * (1.0 - f)));
				}
			}
			return rat;
...
#pragma region Windows Form Designer generated code
		void InitializeComponent(void) {
			...
			this->isSound = (gcnew System::Windows::Forms::CheckBox());
			...
			// result
			// 
			this->result->Location = System::Drawing::Point(874, 134);
			this->result->Margin = System::Windows::Forms::Padding(0);
			this->result->Name = L"result";
			this->result->Size = System::Drawing::Size(314, 268);
			...
			// back
			// 
			this->back->Location = System::Drawing::Point(874, 55);
			...
			// log
			// 
			this->log->Location = System::Drawing::Point(874, 416);
			this->log->Name = L"log";
			this->log->Size = System::Drawing::Size(314, 61);
			this->log->TabIndex = 5;
			...
			this->isSound->Location = System::Drawing::Point(874, 16);
			this->isSound->Name = L"isSound";
			this->isSound->Size = System::Drawing::Size(60, 21);
			this->isSound->TabIndex = 6;
			this->isSound->Text = L"Звук";
			...
			this->Controls->Add(this->isSound);
...
#pragma endregion
	private: System::Void back_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		if (type == 2) progress->Value = progress->Maximum;
	}
	...
	private: System::Void click(System::Windows::Forms::PictureBox^ pb, int n) {
		if (last != -2 && type != 2) { //endgame checker
			if (last == -1) {
				...
				if (game->isEqualColor(st.from, st.to)) {
					...
					if (last != n) {
						...
						if (valid) {
							if(isSound->Checked) step_sound->Play();
							string status = game->getStatus();
							if (status != "ingame") {
								string file = "icons/" + status + ".png";
								System::String^ s = gcnew System::String(file.c_str());
								this->result->Image = Image::FromFile(s); 
								last = -2;
								if (training) {
...
private: System::Void worker_DoWork(System::Object^ sender, System::ComponentModel::DoWorkEventArgs^ e) {
	while (true) {
		...
		if (!e->Cancel) {
			...
			System::String^ s = gcnew System::String(file.c_str());
			this->result->Image = Image::FromFile(s);
			log->Text = "Партия завершена. Идёт тренировка нейросети на данной партии и сохранение весов...";
			game->trainAI(*AI);
			AI->saveW("weights/" + *path);
			progress->Value++;
...
private: System::Void worker_ai_DoWork(System::Object^ sender, System::ComponentModel::DoWorkEventArgs^ e) {
	string status = game->getStatus();
	if (status != "ingame") {
		log->Text = "Партия завершена. Идёт тренировка нейросети на данной партии и сохранение весов...";
		back->Enabled = false;
		...
		AI->saveW("weights/" + *path);
		log->Text = "Готово";
		int new_rat = new_rating(rating, game->resources.size(), game->resources[game->resources.size() - 1], youWhite, status);
		System::IO::File::WriteAllText("rating.chess", System::Convert::ToString(new_rat));
		rl->Text = "Ваш рейтинг: " + System::Convert::ToString(new_rat);
		back->Enabled = true;
	}
	else {
		game->doStepAI(*AI);
